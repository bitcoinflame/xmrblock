...
rpccalls::get_current_height()
{
    ...
    if (!r)
    {
        cerr << "Error connecting to BitcoinFlame daemon at "
             << deamon_url << endl;
        return 0;
    }
...
rpccalls::get_network_info(COMMAND_RPC_GET_INFO::response& response)
{
    ...
    if (r)
    {
        ...
        if (!err.empty())
        {
            cerr << "Error connecting to BitcoinFlame daemon due to "
                 << err << endl;
            return false;
        }
    }
...
rpccalls::get_block(string const& blk_hash, block& blk, string& error_msg)
{
    ...
    {
        cerr << "get_block: error connecting to BitcoinFlame daemon at "
             << deamon_url << endl;
        return false;
    }
